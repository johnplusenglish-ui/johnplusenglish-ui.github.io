/* johnplusenglish - anonymous, aggregate item analytics for the C1 auto-scored pages.
   ...
   Key scheme (must match tools/build-c1-manifest.mjs):
     c1a_<page>_<part>_<set>_<item>_o<idx>   MC: option idx (0-based) chosen
     c1a_<page>_<part>_<set>_<item>_n        typed: attempt
     c1a_<page>_<part>_<set>_<item>_ok       typed: correct attempt
   <set> is the 0-based test/set index; <item> is the 1-based gap/question number. */
(function () {
  var HIT = 'https://abacus.jasoncameron.dev/hit/johnplusenglish/c1a_';
  function ping(counter) {
    try { fetch(HIT + counter, { mode: 'cors', keepalive: true }).catch(function () {}); }
    catch (e) {}
  }
